...
import {
	LocalFileAttachmentManager,
	type LocalFileAttachmentManagerConfig,
} from '../local-file-attachment-manager';
...
describe('LocalFileAttachmentManager', () => {
	...
	describe('constructor', () => {
		it('should create base directory if it does not exist', () => {
			...
			const testManager = new LocalFileAttachmentManager({
	...
	describe('uploadFile', () => {
		...
		it('should calculate checksum correctly', async () => {
			...
			const record2 = await manager.uploadFile(
				Buffer.from('checksum test'),
				'test2.txt',
			);
	...
	describe('deleteFile', () => {
		...
		it('should not throw error when deleting non-existent file', async () => {
			await expect(
				manager.deleteFile('non-existent-file-id'),
	...
	describe('reconstructAttachmentFile', () => {
		it('should reconstruct attachment file from storage metadata', async () => {
			...
			const attachmentFile = manager.reconstructAttachmentFile(
		...
		it('should throw error if storage metadata is invalid', () => {
			expect(() => manager.reconstructAttachmentFile({})).toThrow(
	...
	describe('integration', () => {
		it('should handle multiple files', async () => {
			...
			const file3 = await manager.uploadFile(
				Buffer.from('file 3'),
				'file3.txt',
			);
